...
class _SplitState extends State<Split> {
  ...
  Widget _buildLayout(BuildContext context, BoxConstraints constraints) {
    ...
    double minFractionForIndex0(int index) =>
        minSizeForIndex(index) / availableSize;

    void clampFraction(int index) {
      fractions[index] =
          fractions[index].clamp(minFractionForIndex0(index), 1.0);
    }
    ...
    double deltaFromMinimumSize(int index) =>
        fractions[index] - minFractionForIndex0(index);
    ...
    // display the divider.
    final sizes = List.generate(fractions.length, sizeForIndex);
